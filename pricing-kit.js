...
const productPrice = money.format(config[data.market].price);
document.getElementById("unlock-copy").textContent =
  `${productPrice} one-time · personalised pricing plan, package ideas and negotiation scripts`;
document.getElementById("checkout-preview").addEventListener("click", () => {
  alert("No payment is being collected yet. The next build connects real checkout and post-payment unlock.");
});

// Future payment success can load this page with ?mode=paid after server-side verification.
// We intentionally do not trust a URL parameter alone to unlock paid delivery in the MVP preview.
